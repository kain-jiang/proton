...
func CreateNebula(name string, param *types.NebulaComponentParams) (*types.ComponentNebula, error) {
	// 查询插件是否存在
	...
	// 检查参数（按需，将完善参数）
	if err := checkNebula(name, param, nil); err != nil {
		return nil, cerr.NewError(cerr.ParamsInvalidError, "check nebula params error", err.Error())
	}

	// 插件信息
	plgInfo, err := util.FromMap[types.NebulaPluginConfig](plugin.Config)
	if err != nil {
		return nil, fmt.Errorf("parse plugin error: %w", err)
	}

	crManifest := nebulaManifest(name, param, plgInfo)
	if err := prepareNebula(param); err != nil {
		return nil, fmt.Errorf("prepare nebula error: %w", err)
	}

	global.Logger.WithField("manifest", crManifest).Debug("display nebula manifest")

	// 创建CR
	_, err = global.K8sCli.CustomResourceSet(k8s.GVRNebula, name, param.Namespace, crManifest)
	if err = base.DealK8sStatusError(err); err != nil {
		return nil, fmt.Errorf("set custom resource failed: %w", err)
	}

	componentRelease := &types.ComponentNebula{
		Name:    name,
...
func UpgradeNebula(name string, param *types.NebulaComponentParams) (*types.ComponentNebula, error) {
	// 查询插件是否存在
	plugin, err := global.Persist.GetPluginObject(SelfPluginName)
	if err != nil {
		return nil, fmt.Errorf("get plugin error: %w", err)
	...
	obj, err := cpt.TryToNebula()
	if err != nil {
		return nil, fmt.Errorf("parse component error: %w", err)
	}

	// 检查参数（按需，将完善参数）
	if err := checkNebula(name, param, obj.Params); err != nil {
		return nil, cerr.NewError(cerr.ParamsInvalidError, "check nebula params error", err.Error())
	}
...
func DeleteNebula(name string, toClean bool) (*types.ComponentNebula, error) {
	// 检查组件已存在
	cpt, err := global.Persist.GetComponentObject(name)
	if err != nil {
		return nil, fmt.Errorf("get component error: %w", err)
	...
	nebulaObj, err := cpt.TryToNebula()
	if err != nil {
		return nil, fmt.Errorf("parse component error: %w", err)
	}

	err = global.K8sCli.CustomResourceDelete(k8s.GVRNebula, nebulaObj.Name, nebulaObj.Params.Namespace)
	if err != nil {
	...
	if toClean {
		...
		err := clearNebula(nebulaObj.Params)
		if err != nil {
	...
	return nebulaObj, nil
}

func GetNebula(name string) (*types.ComponentNebula, error) {
	cpt, err := global.Persist.GetComponentObject(name)
	if err != nil {
		return nil, fmt.Errorf("get component error: %w", err)
	}
...
func ListNebula() ([]*types.ComponentNebula, error) {
	// 检查组件已存在
	...
	result := make([]*types.ComponentNebula, 0)
