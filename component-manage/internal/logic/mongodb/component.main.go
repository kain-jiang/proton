...
func CreateMongoDB(name string, param *types.MongoDBComponentParams) (*types.ComponentMongoDB, error) {
	// 查询插件是否存在
	...
	// 检查参数（按需，将完善参数）
	if err := checkMongoDB(name, param, nil); err != nil {
		return nil, cerr.NewError(cerr.ParamsInvalidError, "check mongodb params error", err.Error())
	}

	// 插件信息
	plgInfo, err := util.FromMap[types.MongoDBPluginConfig](plugin.Config)
	if err != nil {
		return nil, fmt.Errorf("parse plugin error: %w", err)
	}

	crManifest := mongoDBManifest(name, param, plgInfo)
	if err := prepareMongoDB(param); err != nil {
		return nil, fmt.Errorf("prepare mongodb error: %w", err)
	}

	global.Logger.WithField("manifest", crManifest).Debug("display mongodb manifest")

	// 创建CR
	_, err = global.K8sCli.CustomResourceSet(k8s.GVRMongoDB, name, param.Namespace, crManifest)
	if err = base.DealK8sStatusError(err); err != nil {
		return nil, fmt.Errorf("set custom resource failed: %w", err)
	}

	componentRelease := &types.ComponentMongoDB{
		Name:    name,
...
func UpgradeMongoDB(name string, param *types.MongoDBComponentParams) (*types.ComponentMongoDB, error) {
	// 查询插件是否存在
	plugin, err := global.Persist.GetPluginObject(SelfPluginName)
	if err != nil {
		return nil, fmt.Errorf("get plugin error: %w", err)
	...
	obj, err := cpt.TryToMongoDB()
	if err != nil {
		return nil, fmt.Errorf("parse component error: %w", err)
	}

	// 检查参数（按需，将完善参数）
	if err := checkMongoDB(name, param, obj.Params); err != nil {
		return nil, cerr.NewError(cerr.ParamsInvalidError, "check mongodb params error", err.Error())
	}
...
func DeleteMongoDB(name string, toClean bool) (*types.ComponentMongoDB, error) {
	// 检查组件已存在
	cpt, err := global.Persist.GetComponentObject(name)
	if err != nil {
		return nil, fmt.Errorf("get component error: %w", err)
	...
	mongodbObj, err := cpt.TryToMongoDB()
	if err != nil {
		return nil, fmt.Errorf("parse component error: %w", err)
	}

	err = global.K8sCli.CustomResourceDelete(k8s.GVRMongoDB, mongodbObj.Name, mongodbObj.Params.Namespace)
	if err != nil {
	...
	if toClean {
		...
		err := clearMongoDB(mongodbObj.Params)
		if err != nil {
	...
	return mongodbObj, nil
}

func GetMongoDB(name string) (*types.ComponentMongoDB, error) {
	cpt, err := global.Persist.GetComponentObject(name)
	if err != nil {
		return nil, fmt.Errorf("get component error: %w", err)
	}
...
func ListMongoDB() ([]*types.ComponentMongoDB, error) {
	// 检查组件已存在
	...
	result := make([]*types.ComponentMongoDB, 0)
