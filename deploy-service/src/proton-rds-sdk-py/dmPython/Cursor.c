...
sdint2
Cursor_SetExecId(
    udt_Cursor*     self    /*IN: cursor to set the rowcount on*/
);
...
Cursor_New2(
    ...
    self = (udt_Cursor*) type->tp_alloc(type, 0);
    if (!self)
		return NULL;

    connection = (udt_Connection*) PyTuple_GetItem(args, 0);
...
)
{
    ...
    {
        udt_var = (udt_Variable*)PyList_GET_ITEM(self->col_variables, size);
        if (udt_var->type == &vt_Object)
        {
            (*udt_var->type->finalizeProc)(udt_var);
        }
    }
    Py_CLEAR(self->col_variables);
...
)
{
    ...
    for (i = 0; i < self->paramCount; i ++)
    {
        ...
        if (Variable_PutDataAftExec(var, (udint4)irow) < 0)
        {
            return -1;
        }
    }
...
)
{
    ...
    for (icol = 0; icol < self->colCount; icol ++)
    {
        ...
        PyList_SET_ITEM(self->col_variables, icol, (PyObject*)udt_var);
    }
...
)
{
    ...
    udt_VariableType*   tmp_varType = NULL;     //��ʱ����
    udint4              size;
    int                 is_udt;
    sdint2              param_type;
    ...
    {       
        ...
        new_var = (udt_Variable*)PyList_GET_ITEM(self->param_variables, iparam);
        ...
        if (new_var == NULL)
        {
            ...
            {
                tmp_varType = Variable_TypeBySQLType(bindParamDesc->sql_type, 1);
                if (tmp_varType == NULL)
                {
                    goto fun_end;
                }
...
)
{
    ...
    if (varType != NULL)
    {
        var = Variable_NewByVarType(self, varType, 1, varType->size);
    }
...
static PyMemberDef g_CursorMembers[] = {
    ...
    { "rownumber",      T_INT,          offsetof(udt_Cursor, rowCount),         READONLY },   /** �α����ڵ�ǰλ��0-based **/
    { "with_rows",      T_BOOL,         offsetof(udt_Cursor, with_rows),        READONLY },   /** �α����ڵ�ǰλ��0-based **/
    ...
    { "lastrowid",      T_OBJECT,       offsetof(udt_Cursor, lastrowid_obj),    RESTRICTED },
    { "execid",         T_OBJECT,       offsetof(udt_Cursor, execid_obj),       READONLY },
