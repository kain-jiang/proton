...
class Test_PooledDB:
    ...
    def test_0(self,host,port,user,password,database1):
        global op
        w = PooledDBInfo(creator=pymysql, host=host, port=int(port), user=user, password=password, database=database1, autocommit=True)
        op = PooledDB(
            master = w,
            backup = w,
        )
    ...
    def test_readWriteSplit(self,host,port,user,password,database1,database2):
        w = PooledDBInfo(creator=pymysql, host=host, port=int(port), user=user, password=password, database=database1)
        r = PooledDBInfo(creator=pymysql, host=host, port=int(port), user=user, password=password, database=database2)
        op = PooledDB(
            master = w,
            backup = r,
        )
