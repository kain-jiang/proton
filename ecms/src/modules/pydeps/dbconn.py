...
from clusterconf import ClusterConfig
...
class DBInfo:
    ...
    说明：host 连接的数据库IP地址
          ...
          passwd 访问数据库的密码
          dbname 访问的数据库名称
          read_timeout mysql连接读超时设置，单位：秒
          write_timeout mysql连接写超时设置，单位：秒
    """
    def __init__(self):
        self.host = ''
        self.port = 0
...
def get_upgrade_db_conn(db_name=""):
    """
    升级使用:获取数据库连接
    @param db_name 数据库名称
    @return 数据库连接
    ...
    功能：使用 PersistentDB 管理连接池,连接均为专用连接
    说明：使用方法如下,注意关闭游标,及连接
          conn = dbconn.get_upgrade_db_conn('ecms') //调用ecms数据库连接的接口
          cursor = conn.cursor()                    //调用当前游标
          cursor.execute(sql)                       //调用execute(sql),执行 sql
          conn.commit()                             //事务性提交
          cursor.close()                            //关闭游标
          conn.close()                              //关闭连接
    """
    if ClusterConfig.if_use_external_db():
        info = ClusterConfig.get_external_db_info()
    else:
        info = dict(
            db_host='127.0.0.1',
            db_port=3306,
    ...
    dbinfo = DBInfo()
    dbinfo.host = info['db_host']
    dbinfo.port = info['db_port']
    dbinfo.user = info['db_user']
    dbinfo.passwd = info['db_password']
    dbinfo.dbname = db_name

    connector = Connector(dbinfo)
    return connector.get_db_conn()


class Connector_with_ssl(Connector):
    """
    功能：提供 SSL 的数据库连接方式
    说明: 使用方法和Connector一样，增加了ssl参数
    """
    def __init__(self, dbinfo):
        super(Connector_with_ssl, self).__init__(dbinfo)
        self.ssl = dbinfo.ssl
