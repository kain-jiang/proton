...
import { MQConnectInfo } from "../../index.d";
...
export class MQConnectInfoBase extends React.Component<Props, State> {
   ...
   * @param mq 连接信息类型
   */
  private initConfig(configData: any) {
   ...
   * @param mq mq信息
   */
  public changeMQConnectInfo(key: string, val: any, mq: any) {
    let cur = mq;
   ...
   * @param mq mq信息
   */
  public changeMQAuthConnectInfo(key: string, val: any, mq: any) {
    const cur = {
      ...mq,
      auth: {
        ...mq.auth,
  ...
  // 是否禁用不可修改信息
  protected getIsDisabled(mq: MQConnectInfo) {
    ...
    if (mq?.source_type === SOURCE_TYPE.INTERNAL) {
      ...
      if (originSourceType !== SOURCE_TYPE.EXTERNAL) {
        ...
        return originConnectInfoType === mq?.mq_type;
      }
    }
  }
}
