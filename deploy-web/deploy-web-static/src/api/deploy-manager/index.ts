...
class AccessAddr {
     ...
     * @param data 配置信息
     */
    async put(type: string, data: Partial<AccessAddress>): Promise<null> {
        return request.put(`${this.url}/${type}`, data);
    }
}

@urlDecorator("/api/deploy-manager/cert")
class Cert {
    url: string;
     ...
     * @param hostname 名称
     */
    async setCert(certType: string, hostname: string): Promise<null> {
        return request.get(
            `${this.url}/set-global-cert/${certType}/${hostname}`
        );
    }
}

class CertInterface {
    url: string = "/interface/deployweb/upload/cert/";

    async upload(certType: string, ip: string, data: any): Promise<null> {
        return request.put(`${this.url}?certType=${certType}&ip=${ip}`, data);
    }
}

export const certDownload = {
    url: "/api/deployweb/deploy-manager/cert/feature/download",
    ...
    get: async function (): Promise<{ status: boolean }> {
        return await request.get(this.url);
    },
};

export const getCertInfo = {
    url: "/api/deployweb/deploy-manager/cert/cert-info",
...
export const certInterface = new CertInterface();
